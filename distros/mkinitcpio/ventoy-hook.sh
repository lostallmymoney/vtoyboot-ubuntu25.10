...
# Shared early-boot helpers start here. This script runs inside mkinitcpio's
...
# The Ventoy metadata device may appear slightly after the hook fires, so wait a
# short time before attempting to construct the device-mapper view.
...
run_hook() {
    # Mount efivarfs if needed, detect whether we are booting through Ventoy, and
    # only then synthesize the mapped block devices that expose the guest image.
    ventoy_check_efivars
