...
# Same idea as grub-probe.sh, but for grub-editenv. vtoyboot records the calls
# issued in the original VM and replays matching results later when the disk path
# has changed under Ventoy boot.
...
if [ -e /dev/mapper/ventoy -a -d /etc/vtoyboot/editenv ]; then
    ...
    # editenv request/result so the later Ventoy-side execution can replay them.
