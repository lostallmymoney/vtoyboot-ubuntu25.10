...
# grub-mkconfig may probe paths that only make sense in the original VM where
# vtoyboot ran. This wrapper records successful grub-probe answers there, then
# replays them later after the disk shows up as /dev/mapper/ventoy inside the
# booted image, translating that path back to /dev/sda when needed.
...
if [ -e /dev/mapper/ventoy -a -d /etc/vtoyboot/probe ]; then
    ...
    # First pass: run on the original guest installation and persist every
    # probe request/result so the later Ventoy-side execution can replay them.
